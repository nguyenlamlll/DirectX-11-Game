...
void DirectXCore::Sprite::LoadTexture(const wchar_t * _charPath)
{
...
void Sprite::Update(float _deltaTime)
{
	...
	if (collider)
	{
		collider->SetColliderScale(Vector3(transform->GetScale().x * (spriterect->right - spriterect->left), transform->GetScale().y * (spriterect->bottom - spriterect->top), 1));
