...
void WizardBoss::Update(float _deltaTime)
{
	if (health > 0)
	{
		...
		if (hurtTime > 0) hurtTime -= _deltaTime;
	}
	ManageAnimation();
	if (dieTime > 0) dieTime -= _deltaTime;
	if (dieTime > 0 && dieTime < 0.5)
	{
...
void WizardBoss::AddAnimation()
{
	...
	Animation* dieAnim = new Animation(L"Resources/Captain/Animations/boss/die.png", "Die", this->GetComponent<Renderer>(), 1, 2, 0.1f, 1.0f, true);
	Animation* hurtAnim = new Animation(L"Resources/Captain/Animations/boss/hurt.png", "Hurt", this->GetComponent<Renderer>(), 1, 1, 0.1f, 1.0f, true);
...
void WizardBoss::ManageAnimation()
{
	...
	else if (health > 0 && hurtTime > 0.3f)
	{
